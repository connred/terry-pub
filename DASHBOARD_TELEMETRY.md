...
## Safeguard Capsule
- Pulled from `/api/safeguards` and rendered inside the System Status card. Shows only high-level info:
  - **State** badge (`Ready / Paused / Blocked`) with a short reason if trading is halted.
  - Remaining **daily trades** and **loss buffer** so operators know how close Terry is to the limits.
  - Flags for **Paper Trading**, **Manual Approval**, and any **Pending Approvals**; no thresholds or wallet data are exposed.
